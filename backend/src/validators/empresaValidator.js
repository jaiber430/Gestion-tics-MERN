...
const empresaValidator = async (data, session) => {
    ...
    // Buscar tipo de empresa directamente en TipoEmpresaRegular
    const existeTipoEmpresa = await TipoEmpresaRegular.findById(tipoEmpresa).session(session)
    ...
    if (!regexEmail.test(emailEmpresa)) {
