...
// ===============================
// FUNCION PARA COMBINAR PDFs
// ===============================
const combinarPdfs = async (carpeta) => {
    try {
        const archivos = fs.readdirSync(carpeta)
        const pdfs = archivos
            .filter(archivo => archivo.endsWith('.pdf') && archivo !== 'combinado.pdf')
            .sort((a, b) => {
                const statA = fs.statSync(path.join(carpeta, a))
                const statB = fs.statSync(path.join(carpeta, b))
                return statA.birthtimeMs - statB.birthtimeMs  // orden de creación
            })

        // Si hay uno o ninguno, no hace nada
...
// ===============================
// MIDDLEWARE PARA COMBINAR DESPUES DE SUBIR
// ===============================
