...
const RevisarSolicitudesCoordinador = ({ ofertas }) => {
    ...
    useEffect(() => {
        const obtnerRevisiones = async () => {
            try {
                const [revisonRes] = await Promise.all([
                    clienteAxios.get('/consultas/obtener-revisiones-coordinador')
                ])
                setRevision(revisonRes.data)
            } catch (error) {
                setAlerta({
                    msg: error?.response?.data?.msg || 'Error al cargar las solicitudes',
                    error: true
                })
            }
        }
    ...
    const handleSubmit = async (idSolicitud, e) => {
        e.preventDefault()
        try {
            const { data } = await clienteAxios.put(`/consultas/revision-coordinador/${idSolicitud}`, { estado, observacion })
            setAlerta({
                msg: data,
                error: false
            })
    ...
    return (
        <div className="space-y-6">
            ...
            {ofertas.map((o, index) => {
                ...
                // Busca la revisión que corresponde a esta oferta específica
                const revisionOferta = revision.find(r => r.solicitud === o._id)
                // Solo deshabilita si fue APROBADO — rechazado sigue editable
                const yaAprobado = revisionOferta?.estado === true
                ...
                return (
                    ...
                    >
                        ...
                        <div className="pl-6 p-6">
                            ...
                            <form onSubmit={(e) => handleSubmit(o._id, e)}>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                                    ...
                                    <div className="bg-slate-50 rounded-xl p-4 border border-slate-100">
                                        ...
                                        <select
                                            value={yaAprobado ? 'true' : (estado === undefined ? '' : String(estado))}
                                            ...
                                            className={`w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:border-green-400 focus:ring-4 focus:ring-green-50 transition-all bg-white text-slate-700
                                                ${yaAprobado ? 'opacity-60 cursor-not-allowed bg-slate-100' : ''}`}>
                                    ...
                                    <div className="bg-slate-50 rounded-xl p-4 border border-slate-100">
                                        ...
                                        <textarea
                                            ...
                                            disabled={estado !== false || yaAprobado}
                                ...
                                <div className="pt-4 border-t border-slate-200 flex justify-end items-center gap-2">
                                    ...
                                    {/* Botón — si ya aprobado queda fijo verde deshabilitado, sino normal */}
                                    {yaAprobado ? (
                                        <button
                                            disabled
                                            className="px-4 py-2 text-sm rounded-xl flex items-center gap-2 font-medium shadow-md bg-gradient-to-r from-green-600 to-emerald-600 text-white shadow-green-200 cursor-not-allowed opacity-70">
                                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <button
                                            type='submit'
                                            disabled={estado === '' || estado === undefined}
